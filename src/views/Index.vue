        ...
        methods: {
            submitForm(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        axios.post('http://localhost:8181/account/submit',this.ruleForm).then(function(resp){
                            if(resp.data == 'success'){
                                axios.get('http://localhost:8181/account/findByName/'+(_this.ruleForm.account_name)).then(function (resp2){
